...
public class DFRegisterAgent extends Agent {
  ...
  protected void setup() {
  	String galleryName = "unknown";
  	
  	// Read the name of the service to register as an argument
  	Object[] args = getArguments();
  	if (args != null && args.length > 0) {
  		galleryName = (String) args[0];
  	}
  	
  	// Register the service
  	System.out.println("Agent "+getLocalName()+" registering service \""+galleryName+"\" of type \"art\"");
  	try {
  		DFAgentDescription dfd = new DFAgentDescription();
  		dfd.setName(getAID());
  		ServiceDescription sd = new ServiceDescription();
  		sd.setName(galleryName);
  		sd.setType("photo");
  		// Agents that want to use this service need to "know" the weather-forecast-ontology
  		sd.addOntologies("photo-ontology");
